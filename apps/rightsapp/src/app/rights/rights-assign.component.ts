...
export class RightsAssignComponent {
  ...
  private routeContentId = toSignal(this.route.paramMap.pipe(map((p) => p.get('contentId'))));
  ...
  onRegionChange(region: Region, e: Event) {
    ...
    const next = checked ? [...current, region] : current.filter((r) => r !== region);
  ...
  onSubmit() {
    ...
    const contentId = paramId && paramId !== 'new' ? paramId : (value.contentIdInput ?? '').trim();
    if (!contentId) return;
