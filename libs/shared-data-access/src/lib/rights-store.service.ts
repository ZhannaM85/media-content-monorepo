...
export class RightsStoreService {
  ...
  getRightsByContentId(contentId: number | string): Observable<Rights | undefined> {
    return this.rights$.pipe(
      map((list) => list.find((r) => String(r.contentId) === String(contentId)))
    );
  }

  addOrUpdateRights(rights: Rights): void {
    const list = this.rights$.value;
    const index = list.findIndex((r) => r.id === rights.id || String(r.contentId) === String(rights.contentId));
    const next = index >= 0 ? [...list.slice(0, index), rights, ...list.slice(index + 1)] : [...list, rights];
